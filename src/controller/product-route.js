...
import { addProductImage, getProductImage } from "../service/product.service.js";
...
router.get('/:productId/image', async (req, res) => {
  try {
    const fileStream = await getProductImage(req.params.productId);
...
router.put("/:id", async (request, response) => {
  const productId = request.params.id;
  const aProduct = request.body;
  await productRepository.changeProductInfo(productId, aProduct);
